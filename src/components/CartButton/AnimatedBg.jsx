...
function drawScissor(ctx, cx, cy, sz, ang, open, dark) {
  ...
  ctx.save();
  ctx.rotate(-a);
  drawBlade(ctx, sz, sz * 0.55, w, dark);
  ctx.restore();
  ctx.save();
  ctx.rotate(a);
  ctx.scale(1, -1);
...
function renderBg(ctx, tools, W, H, t) {
  ...
  for (const tool of sorted) {
    tool.x += tool.dx;
    tool.y += tool.dy;
    tool.angle += tool.aSpeed * 0.016;
    if (tool.x < -0.08) tool.x = 1.08;
    if (tool.x > 1.08) tool.x = -0.08;
    if (tool.y < -0.08) tool.y = WAVE_Y - 0.02;
    if (tool.y > WAVE_Y - 0.015) tool.y = 0.02;
    ctx.save();
    ctx.globalAlpha = tool.alpha * (0.6 + tool.layer * 0.55);
    ctx.fillStyle = GOLDS[tool.ci];
    ctx.strokeStyle = GOLDS[tool.ci];
    ctx.lineWidth = 0.8;
  ...
  grd.addColorStop(0, "hsla(44,92%,68%,.95)");
  grd.addColorStop(0.06, "hsla(41,87%,54%,.98)");
  grd.addColorStop(0.18, "hsla(39,82%,43%,1)");
  grd.addColorStop(0.4, "hsla(36,76%,32%,1)");
  grd.addColorStop(0.7, "hsla(33,70%,20%,1)");
...
export function AnimatedBg({ style }) {
  ...
  useEffect(() => {
    ...
    function loop(ts) {
      renderBg(ctx, toolsRef.current, W, H, ts * 0.001);
