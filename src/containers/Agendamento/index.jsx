...
export function Agendamento() {
  ...
  const fetchHorarios = useCallback(async () => {
    ...
    try {
      ...
      if (
        Array.isArray(dadosHorarios) &&
        dadosHorarios.length > 0 &&
        typeof dadosHorarios[0] === "string"
      ) {
      ...
      console.error("Erro ao buscar horários da API:");
  ...
  // Restaura a seleção que o usuário fez ANTES de ser mandado pro login
  useEffect(() => {
    if (userInfo && location.state?.pendingSelection) {
      const { barberId, date, time, monthlyPlan } =
        location.state.pendingSelection;
      if (barberId) setSelectedBarber(barberId);
      if (date) setSelectedDate(date);
      if (time) setSelectedTime(time);
      if (monthlyPlan) setIsMonthlyPlan(monthlyPlan);

      // Limpa o state da navegação pra não ficar restaurando de novo
      // em re-renders futuros (ex: usuário aperta F5)
  ...
  const handleConfirmarAgendamento = async () => {
    ...
    if (!userInfo) {
      const irParaLogin = window.confirm(
        "Você precisa estar logado para confirmar o agendamento. Deseja ir para a tela de login agora?",
      );
      if (irParaLogin) {
        navigate(barbershopSlug ? `/${barbershopSlug}/login` : "/", {
          state: {
            from: "/app/agendamento",
            pendingSelection: {
    ...
    if (
      ...
      (selectedDate === todayStr && selectedTime < currentTimeStr)
    ) {
      toast.error(
        "Você selecionou um horário que já passou! Escolha outra opção.",
      );
      return;
    }
      ...
      <S.ContentWrapper>
        ...
        {isEmployee && (
          ...
          >
            <S.AdminTextWrapper>
              ...
              <S.AdminDescription>
                Caso esteja agendando para um cliente presencial, informe o nome
                e telefone abaixo. **Se deixar em branco, o agendamento será
                feito em seu próprio nome.**
              </S.AdminDescription>
            </S.AdminTextWrapper>
            ...
            <div style={{ marginTop: "8px" }}>
              <S.AdminDescription style={{ marginBottom: "8px" }}>
                **Plano Mensal Recorrente:** Aplica descontos automáticos de
                Segunda a Quarta.
              </S.AdminDescription>
              <S.CheckboxLabel>
                <S.StyledCheckbox
        ...
        {selectedDate && selectedBarber && (
          <div style={{ marginBottom: 45 }}>
            ...
            {loadingTimes && (
              <S.StatusText>
                Carregando horários livres do sistema...
              </S.StatusText>
            )}

            {!loadingTimes && availableTimes.length === 0 && (
              <S.StatusText>
                Nenhuma jornada configurada ou horários esgotados para este dia.
              </S.StatusText>
            )}
