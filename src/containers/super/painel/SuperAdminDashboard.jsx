...
export function SuperAdminDashboard() {
    ...
    async function handleCreateBarbershop(e) {
        ...
        try {
            ...
            alert("Erro ao criar barbearia: " + (err.response?.data?.message || err.response?.data?.error || err.message));
        }
    }
    ...
    async function handleSaveAdmin(e) {
        ...
        try {
            ...
            alert("Erro ao salvar admin: " + (err.response?.data?.error || err.message));
        }
    }
    ...
    async function handleDeleteAdmin(shopId) {
        if (!confirm("Tem certeza que deseja remover o administrador desta barbearia?")) return;
        ...
        try {
            ...
            alert("Erro ao excluir admin: " + (err.response?.data?.error || err.message));
        }
    }
    ...
    async function handleToggleStatus(shopId, currentStatus) {
        ...
        if (!confirm(`Tem certeza que deseja ${actionName} esta barbearia?`)) return;
    ...
    if (loading) return <Container><p>Carregando painel do SuperAdmin...</p></Container>;
    ...
    return (
        <Container>
            ...
            <GridSection>
                ...
                <Card>
                    <CardHeader>
                        <h2>{isEditingAdmin ? "Editar Admin da Barbearia" : "Cadastrar Admin da Barbearia"}</h2>
                        {isEditingAdmin && (
                    ...
                    <Form onSubmit={handleSaveAdmin}>
                        ...
                        <Input
                            type="password"
                            placeholder={isEditingAdmin ? "Nova Senha (deixe em branco para manter)" : "Senha de Acesso"}
                            value={adminPassword}
                        ...
                        <SubmitButton type="submit" $isEditing={isEditingAdmin}>
                            {isEditingAdmin ? "Salvar Alterações do Admin" : "Criar Administrador"}
                        </SubmitButton>
                    </Form>
                </Card>
            </GridSection>
