  ...
  return (
    <Container>
      <MenuNav>
        <NavButton
          type="button"
          $active={subAba === "barbeiros"}
          onClick={() => setSubAba("barbeiros")}
        >
          Barbeiros
        </NavButton>
        <NavButton
          type="button"
          $active={subAba === "servicos"}
          onClick={() => setSubAba("servicos")}
        >
          Serviços
        </NavButton>
        <NavButton
          type="button"
          $active={subAba === "categorias"}
              ...
              {barbeiros.map((b) => (
                <tr key={b.id}>
                  ...
                  <TdCenter>
                    <ActionButton
                      $variant="editar"
                      onClick={() => abrirModalEdicao("barbeiros", b)}
                    >
                      Editar
                    </ActionButton>
                    <ActionButton
                      $variant="excluir"
                      onClick={() => handleExcluirItem("barbeiros", b.id)}
                    >
                      Excluir
                    </ActionButton>
                  </TdCenter>
            ...
            <tbody>
              {servicos.map((s) => (
                <tr key={s.id}>
                  ...
                  <Td>
                    {s.is_monthly_offer
                      ? `${Number(s.discount_percentage)}% OFF`
                      : "Não"}
                  </Td>
                  <TdCenter>
                    <ActionButton
                      $variant="editar"
                      onClick={() => abrirModalEdicao("servicos", s)}
                    >
                      Editar
                    </ActionButton>
                    <ActionButton
                      $variant="excluir"
                      onClick={() => handleExcluirItem("servicos", s.id)}
                    >
                      Excluir
                    </ActionButton>
                  </TdCenter>
              ...
              {categorias.map((c) => (
                <tr key={c.id}>
                  ...
                  <TdCenter>
                    <ActionButton
                      $variant="editar"
                      onClick={() => abrirModalEdicao("categorias", c)}
                    >
                      Editar
                    </ActionButton>
                    <ActionButton
                      $variant="excluir"
                      onClick={() => handleExcluirItem("categorias", c.id)}
                    >
                      Excluir
                    </ActionButton>
                  </TdCenter>
      ...
      {subAba === "relatorios" && (
        <div>
          ...
          {relatorio && (
            <div>
              <CardGrid>
                {/* 💰 Já faturado de verdade (status completed) */}
                <Card $highlight>
                  <CardTitle $highlight>Faturamento Realizado</CardTitle>
                  <CardValue $large>
                    R${" "}
                    {Number(relatorio.serviços?.faturamento_realizado || 0)
                      .toFixed(2)
                      .replace(".", ",")}
                  </CardValue>
                </Card>
                <Card>
                  ...
                  <CardValue>
                    {relatorio.serviços?.total_concluidos || 0}
                  </CardValue>
                </Card>

                {/* 📅 Confirmado, mas ainda não aconteceu (status scheduled) */}
                <Card>
                  <CardTitle>A Receber (Agendados)</CardTitle>
                  <CardValue style={{ color: "#2196F3" }}>
                    R${" "}
                    {Number(relatorio.serviços?.valor_a_receber || 0)
                      .toFixed(2)
                      .replace(".", ",")}
                  </CardValue>
                </Card>
                <Card>
                  <CardTitle>Total de Agendamentos Futuros</CardTitle>
                  <CardValue>
                    {relatorio.serviços?.total_agendados || 0}
                  </CardValue>
                </Card>
                ...
                <Card>
                  ...
                  <CardValue style={{ color: "#e53935" }}>
                    {relatorio.serviços?.total_cancelados || 0}
                  </CardValue>
                </Card>
                <Card>
                  <CardTitle>Valor Perdido (Cancelados)</CardTitle>
                  <CardValue style={{ color: "#e53935" }}>
                    R${" "}
                    {Number(relatorio.serviços?.valor_cancelados || 0)
                      .toFixed(2)
                      .replace(".", ",")}
                  </CardValue>
                </Card>
                ...
                {relatorio.produtos && (
                  <Card>
                    ...
                    <CardValue>
                      R${" "}
                      {Number(relatorio.produtos?.faturamento_produtos || 0)
                        .toFixed(2)
                        .replace(".", ",")}
                    </CardValue>
                  </Card>
                ...
                <Card $highlight>
                  <CardTitle $highlight>
                    Faturamento Absoluto (Realizado)
                  </CardTitle>
                  <CardValue $large>
                    R${" "}
                    {Number(relatorio.faturamento_absoluto_barbearia || 0)
                      .toFixed(2)
                      .replace(".", ",")}
                  </CardValue>
                </Card>
                  ...
                  <InputGroup>
                    ...
                    <ModalSelect
                      ...
                      onChange={(e) =>
                        setFormEdicao({
                          ...formEdicao,
                          is_monthly_offer: e.target.value === "true",
                        })
                      }
                    >
                  ...
                  {Boolean(formEdicao.is_monthly_offer) && (
                    <InputGroup>
                      ...
                      <ModalInput
                        ...
                        min="0"
                        max="100"
                        value={formEdicao.discount_percentage || ""}
                        onChange={(e) =>
                          setFormEdicao({
                            ...formEdicao,
